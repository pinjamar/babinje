...
class MailBuilder:
    ...
    def generate_message(self, name: str, isRegister: bool, item_name: str, url: str):
        akcija_tekst = "Hvala što ste se registrirali za " + item_name if isRegister else "Žao nam je što ste odustali od " + item_name
        return self.template.replace("#POZDRAV#", "Bok, " + name).replace("#AKCIJA#", akcija_tekst).replace("#HREF#", url)
