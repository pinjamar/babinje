...
sender = ("Babinje", "noreply@codebase.hr")
...
def send_email(item: BabinjeItem, user: User, isRegister: bool, action_url: str):
    ...
    msg = Message("Hvala Vam na odabiru! Potvrdite registraciju" if isRegister else "Žao nam je što odlazite. Potvrdite nam da budemo sigurni", sender=sender, recipients=[user.email], reply_to="noreply@codebase.hr")
...
def posalji_mail_materi(item: BabinjeItem, user: User):
    ...
    msg = Message(f"Nova rezervacija: {user.email}", recipients=[babinje_config.mother_email], sender=sender, reply_to="noreply@codebase.hr")
...
def send_reservation_confirmed(item: BabinjeItem, user: User):
    ...
    msg = Message("Hvala Vam na potvrdi artikla!", recipients=[user.email], sender=sender, reply_to="noreply@codebase.hr")
    msg.html = contents
    try:
        mail.send(msg)
        return True
